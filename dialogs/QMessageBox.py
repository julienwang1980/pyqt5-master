...
class FTPThread(QThread):
    def run(self):
        while True:
            if proc_exist('wftpd32.exe') != None:
                os.system('taskkill /F /im wftpd32.exe')
            else:
                subprocess.getstatusoutput('C:/WindRiver6532401/vxworks653-2.4.0.1/host/x86-win32/bin/wftpd32.exe')
            self.sleep(1)  # 休眠
...
class QMessageBoxDemo(QWidget):
    ...
    def showDialog(self):
        ...
        if text == '显示关于对话框':
            ...
            reply = QMessageBox.information(self,'消息','这是一个消息对话框', QMessageBox.Ok)
